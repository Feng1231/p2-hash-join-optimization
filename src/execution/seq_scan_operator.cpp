...
// Original constructors - these must exist
...
SeqScanOperator::SeqScanOperator(const ExecutionContext &exec_ctx, const std::string &table_name,
    ...
    : Operator(exec_ctx, {}, output_schema), table_name_(table_name), fetch_columns_(fetch_columns), next_row_id(0) {}
...
OperatorState SeqScanOperator::Next(Chunk &output_chunk) {
    ...
    // Reuse buffer - clear but keep capacity
    reusable_buffer_.clear();
    Chunk& local_buffer = reusable_buffer_;
