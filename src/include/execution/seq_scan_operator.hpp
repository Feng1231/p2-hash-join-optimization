...
OperatorState SeqScanOperator::Next(Chunk &output_chunk) {
    ...
    while (output_chunk.size() < exec_ctx_.config_.CHUNK_SUGGEST_SIZE) {
        ...
        // Create shared tuple - this is the ONLY copy from storage
        auto shared_tuple = std::make_shared<const Tuple>();
        shared_tuple->reserve(key_attrs.size());
        for (auto attr : key_attrs) {
            shared_tuple->push_back(tuple[attr]);
        }
        
        output_chunk.emplace_back(std::move(shared_tuple), next_row_id - 1);
    }
